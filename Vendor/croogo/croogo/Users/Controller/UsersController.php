 ...
 */
	public function index() {
		$this->set('title_for_layout', __d('croogo', $this->Session->read('Auth.User.name')));
	}
 ...
 */
	public function edit() {
		if (!empty($this->request->data)) {
			if ($this->User->saveAssociated($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Updating profile successfully!'), 'flash', array('class' => 'alert alert-success alert-dismissable'));
				return $this->Croogo->redirect(array('plugin' => 'users','controller' => 'users','action' => 'index'));
			} else {
				$this->Session->setFlash(__d('croogo', 'The User could not be saved. Please, try again.'), 'flash', array('class' => 'error'));
			}
		} else {
 ...
 */
	...
	public function reset_password($id = null) {
		...
		if (!empty($this->request->data)) {
			if ($this->User->save($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Changed password successfully!'), 'flash', array('class' => 'alert alert-success alert-dismissable'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__d('croogo', 'Password could not be reset. Please, try again.'), 'flash', array('class' => 'error'));
			}
 ...
 */
	public function view($username = null) {
		...
		$this->User->Task->recursive = 1;
		$tasks = $this->User->Task->find('all', array(
			'conditions' => array(
		...
		//pr($tasks);
		$errors = $this->User->Task->Error->find('all', array(
			'conditions' => array(
				'Task.user_id' => $user['User']['id']
			),
			'fields' => array(
		...
		//pr($errors); die();
		$this->set(compact('user', 'tasks', 'errors'));
	}
	// Filter user by role id
	public function ajax($group_id = NULL)
	{
		$this->layout = false;
		...
		if(!empty($users)){
       		echo "<option value='' class='a'>".__d('croogo', 'Select user')."</option>";
	        foreach($users as $u){
	            echo "<option value='".$u['User']['id']."'>".__($u['User']['name'],true)."</option>";
	        } 
    	}else{
    		echo "<option value=''>". __d('croogo', 'Select user')."</option>";
    		foreach($users as $u){
	            echo "<option value='".$u['User']['id']."'>".__($u['User']['name'],true)."</option>";
	        } 
    	}
